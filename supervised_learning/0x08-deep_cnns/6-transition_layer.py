#!/usr/bin/env python3
""" Module that contains transition_layer function """
...
def transition_layer(X, nb_filters, compression):
    ...
    Return: output of the transition layer and the number of filters within
    the output
    """
    ...
    conv_1x1 = K.layers.Conv2D(
        filters=nb_filters * compression, kernel_size=(1, 1), padding='same',
        kernel_initializer=initializer_kernel_weights)(activated_output)
    average_pooling = K.layers.AveragePooling2D(
        pool_size=(2, 2), strides=(2, 2), padding='same')(conv_1x1)
