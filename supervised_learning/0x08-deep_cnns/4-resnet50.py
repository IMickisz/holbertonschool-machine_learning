#!/usr/bin/env python3
""" Module that contains resnet50 function """
...
def resnet50():
    ...
    output_layer = K.layers.Conv2D(
        ...
        kernel_initializer=initializer_kernel_weights)(inputs)
    ...
    output_layer = K.layers.MaxPooling2D(
        pool_size=(3, 3), strides=(2, 2), padding='same')(activated_output)
    ...
    output_layer = K.layers.AveragePooling2D(
        pool_size=(7, 7), strides=None, padding='same')(activated_output)

    output = K.layers.Dense(
        units=1000, activation='softmax',
        kernel_initializer=initializer_kernel_weights)(output_layer)
