...
class Neuron:
    ...
    def train(self, X, Y, iterations=5000, alpha=0.05, verbose=True,
              graph=True, step=100):
        """
        Method that trains the neuron by updating the private attributes __W,
        __b, and __A
            - X : a np array with shape (nx, m) that contains the input data
            - Y : a np array with shape (1, m) with correct labels
        ...
        output of the neuron for each example
            - alpha : learning rate
            - verbose: boolean to define if print information about training
            - graph: boolean to define if graph information about training
            - step: step iteration to show information
        """
