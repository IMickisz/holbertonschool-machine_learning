#!/usr/bin/env python3
""" Module that contains lenet5 function """
...
def lenet5(X):
    ...
    fully_connected_layer_1 = K.layers.Dense(
        units=120, activation='relu', kernel_initializer=init_kernel_weights)
    output = fully_connected_layer_1(output)

    # Fully connected layer with 84 nodes
    fully_connected_layer_2 = K.layers.Dense(
        units=84, activation='relu', kernel_initializer=init_kernel_weights)
    output = fully_connected_layer_2(output)

    # Fully connected softmax output layer with 10 nodes
    fully_connected_layer_3 = K.layers.Dense(
        units=10, activation='softmax', kernel_initializer=init_kernel_weights)
