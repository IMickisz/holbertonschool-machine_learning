#!/usr/bin/env python3
""" Module that contains conv_forward function """
import numpy as np
...
def conv_forward(A_prev, W, b, activation, padding="same", stride=(1, 1)):
    ...
    kh, kw, c_new = W.shape[0], W.shape[1], W.shape[3]
    sh, sw = stride
    ...
    convolved_images = np.zeros(shape=(m, convolved_image_h,
                                       convolved_image_w, c_new))
        ...
        for width in range(convolved_image_w):
            for kernel in range(c_new):
                convolved_image = np.sum(np.multiply(
                    padded_images[:, height * sh:height * sh + kh,
