#!/usr/bin/env python3
""" Module that contains convolve function """
import numpy as np


def convolve(images, kernels, padding='same', stride=(1, 1)):
    """ Function that performs a convolution with images using multiple kernels
    ...
    for i in range(nc):
        for w in range(W_out):
            for h in range(H_out):
                ...
                output_matrix[:, h, w, i] = np.tensordot(part_image,
                                                         kernels[:, :, :, i],
                                                         axes=3)
